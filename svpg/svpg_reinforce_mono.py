...
class SVPG_Reinforce_Mono(Algo):
    ...
    def compute_reinforce_loss(self, reward, action_logprobs, critic, entropy, done):
        ...
        # Create mask to mask useless values (values that are enregistred into the workspace after the episode end)
        ...
        # cumulated_reward[t] = reward[t] + discount_factor * reward[t+1] + ... + (discount_factor ^ episode_length) * reward[episode_length] 
        cumulated_reward = [torch.zeros_like(reward[-1])]
        for t in range(max_trajectories_length - 1, 0, -1):
            cumulated_reward.append(self.discount_factor *  cumulated_reward[-1] + reward[t])
