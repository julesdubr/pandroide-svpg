...
def updateNovelty(
    ...
    if len(offspring) < _lambda:
        print(
            "ERROR: updateNovelty, lambda(%d)<offspring size (%d)"
        ...
        if verbose:
            ...
            for offs in soff[ilast : len(soff)]:
                print(
                    ...
                    + str(offs.novelty)
                    + " fit="
                    + str(offs.fitness.values)
                    + " bd="
                    + str(offs.bd)
                )
    else:
        print(
            'ERROR: updateNovelty: unknown add strategy(%s), valid alternatives are "random" and "novel"'
            % (add_strategy)
        )
        return None
